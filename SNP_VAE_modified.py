...
def load_data(self, path, mitoSNP_mask, AD, DP, vcf, variant_name):
    ...
    if path != None:
        ...
        AD_raw = mmread(path + '/cellSNP.tag.AD.mtx').toarray()
        DP_raw = mmread(path + '/cellSNP.tag.DP.mtx').toarray()
    ...
    if vcf_raw != None:
        ...
        AD_raw = AD_raw[mitoSNP_filter, :].T
        DP_raw = DP_raw[mitoSNP_filter, :].T
        variant_name = variant_name[mitoSNP_filter]
...
def filter_data(self, cell_SNPread_threshold = None, SNP_DPmean_threshold = None, SNP_logit_var_threshold = None, figure_path = "./"):
    ...
    plt.xlabel("Cell")
    plt.savefig(figure_path + "Cells sorted by number of observed SNPs.png")
    plt.show()
    ...
    plt.xlabel("SNP")
    plt.savefig(figure_path + "Mean coverage of SNPs per high-quality cell.png")
    plt.show()
    ...
    plt.xlabel("SNP")
    plt.savefig(figure_path + "SNPs sorted by logit-variance.png")
    plt.show()
    ...
    vcf_filtered = None
    if self.vcf_raw != None:
        
        vcf_filtered = self.vcf_raw.iloc[SNP_filter, :]

        if self.variant_name == None:
            self.variant_name = "chr:" + vcf_filtered["CHROM"].astype(str) + ", " + vcf_filtered["POS"].astype(str) + vcf_filtered["REF"] + ">" + vcf_filtered["ALT"]
...
def train_VAE(self, num_epoch, stepsize, z_dim, beta, num_batch):
    ...
    pca = PCA()
    pca.fit(self.latent)              ## calculate PC
    pc = pca.fit_transform(self.latent)            ## represent with PC
    
    reducer_2d = umap.UMAP(n_components = 2)
    embedding_2d = reducer_2d.fit_transform(self.latent)     ## get UMAP from latent demension directly
    pair_embedding_2d = squareform(pdist(embedding_2d))      ## calculate pairwise distance
...
def tree(self, cluster_no, pair_no, SNP_no):
    ...
    fig = sns.clustermap(pd.DataFrame(AF_sorted[:SNP_no, :], index = self.variant_name.to_numpy()[rank_SNP][:SNP_no], columns = np.arange(1, self.cell_total + 1)), row_cluster = False, col_cluster = False, col_colors = clus_colors, figsize = (20, SNP_no * 0.6))
...
def summary_phylogeny(self, SNP_no, dpi):
    ...
    clus_colors.index = pd.RangeIndex(start = 1, stop = self.cell_total + 1, step = 1)
    fig = sns.clustermap(pd.DataFrame(self.AF_sorted[:SNP_no, :], index = self.variant_name.to_numpy()[self.rank_SNP][:SNP_no], columns = np.arange(1, self.cell_total + 1)), row_cluster = False, col_cluster = False, col_colors = clus_colors, figsize = (20, SNP_no * 0.6))
...
class SNP_VAE:
    ...
    def filtering(self, cell_SNPread_threshold = None, SNP_DPmean_threshold = None, SNP_logit_var_threshold = None, figure_path = "./"):
        
        filter_data(self, cell_SNPread_threshold = cell_SNPread_threshold, SNP_DPmean_threshold = SNP_DPmean_threshold, SNP_logit_var_threshold = SNP_logit_var_threshold, figure_path = figure_path)
